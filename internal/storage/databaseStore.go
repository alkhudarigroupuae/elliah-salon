...
const (
	...
	addLogoColumnSQL    = `ALTER TABLE config ADD COLUMN IF NOT EXISTS logo_url TEXT DEFAULT '';`
	addFaviconColumnSQL = `ALTER TABLE config ADD COLUMN IF NOT EXISTS favicon_url TEXT DEFAULT '';`
)
...
func createTables(db *sql.DB) error {
	...
	// Migration: add branding columns if they don't exist (for existing DBs)
	for _, q := range []string{addLogoColumnSQL, addFaviconColumnSQL} {
		if _, err := db.Exec(q); err != nil {
			log.Printf("Warning: could not add branding column (may already exist): %v", err)
		}
	}
	return nil
}
...
func (s *databaseStore) saveConfig(config *Config) error {
	...
	query := `
		INSERT INTO config (id, categories, currency, start_date, logo_url, favicon_url)
	...
	`
	_, err = s.db.Exec(query, string(categoriesJSON), config.Currency, config.StartDate, config.LogoURL, config.FaviconURL)
...
func (s *databaseStore) GetConfig() (*Config, error) {
	query := `SELECT categories, currency, start_date, COALESCE(logo_url,''), COALESCE(favicon_url,'') FROM config WHERE id = 'default'`
	var categoriesStr, currency, logoURL, faviconURL string
	var startDate int
	err := s.db.QueryRow(query).Scan(&categoriesStr, &currency, &startDate, &logoURL, &faviconURL)
...
func (s *databaseStore) UpdateBranding(logoURL, faviconURL string) error {
	return s.updateConfig(func(c *Config) error {
		c.LogoURL = strings.TrimSpace(logoURL)
		c.FaviconURL = strings.TrimSpace(faviconURL)
		return nil
	})
}
