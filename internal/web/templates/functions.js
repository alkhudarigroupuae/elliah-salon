...
const DEFAULT_LOGO_URL = 'https://elliahsalon.com/wp-content/uploads/2025/08/logo-elliah-salon-jvc-.svg';

async function applyBrandingFromConfig() {
    try {
        const r = await fetch('/config');
        if (!r.ok) return;
        const c = await r.json();
        const logoUrl = (c.logoUrl && c.logoUrl.trim()) ? c.logoUrl.trim() : DEFAULT_LOGO_URL;
        const faviconUrl = (c.faviconUrl && c.faviconUrl.trim()) ? c.faviconUrl.trim() : logoUrl;
        const logoEl = document.getElementById('headerLogo');
        if (logoEl) logoEl.src = logoUrl;
        document.querySelectorAll('link[rel="icon"], link[rel="apple-touch-icon"]').forEach(link => { link.href = faviconUrl; });
    } catch (e) { /* ignore */ }
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => applyBrandingFromConfig());
} else {
