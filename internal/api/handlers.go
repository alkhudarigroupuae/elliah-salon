...
func (h *Handler) UpdateBranding(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.storage.UpdateBranding(body.LogoURL, body.FaviconURL); err != nil {
		writeJSON(w, http.StatusInternalServerError, ErrorResponse{Error: "Failed to update branding"})
		log.Printf("API ERROR: Failed to update branding: %v\n", err)
		return
	}
	writeJSON(w, http.StatusOK, map[string]string{"status": "success"})
}
